...
const DataTable: React.FC<DataTableProps> = ({ files, selectedKeys, keyGroups }) => {
    const sortedKeys = Array.from(selectedKeys).sort();

    const findValueForDisplayKey = (row: Record<string, string>, displayKey: string): string => {
        const originalKeys = keyGroups.get(displayKey);
        if (originalKeys) {
        ...
        // Fallback for non-grouped keys or if something goes wrong
        if (Object.prototype.hasOwnProperty.call(row, displayKey)) {
             return row[displayKey] ?? 'N/A';
        }
        return 'N/A';
    };
                    ...
                    <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                        {files.map(file => (
                            file.data?.map((row, rowIndex) => (
                                <tr key={`${file.id}-${rowIndex}`} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
